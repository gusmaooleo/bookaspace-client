...
class UserService {
  ...
  async getAll(): Promise<User[] | any> {
    try {
      const payload = await axios.get<{ content: User[] }>(
        `${environments.url}/users`,
        {
          headers: {
  ...
  async requestRecoverPassword(data: { email: string }) {
    try {
      const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      ...
      return await axios.post(`${environments.url}/recover-token`, data, {
        headers: {
          "Content-Type": "application/json",
        },
      });
  ...
  async validateRecoverToken(token: { token: string }) {
    try {
      return await axios.get(`${environments.url}/validate-token`, {
        params: token,
        headers: {
          "Content-Type": "application/json",
        },
  ...
  async changePassword(body: { password: string, token: string }): Promise<AxiosResponse> {
    try {
      return await axios.put(`${environments.url}/new-password`, body, {
        headers: {
          "Content-Type": "application/json",
        },
      });
  ...
  getOABasic(): string {
    return Buffer.from(
      `${environments.username}:${environments.password}`,
      "utf-8"
    ).toString("base64");
  }
}
